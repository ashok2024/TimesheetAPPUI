...
export const getPaginatedTasksByProjectId = async (projectId, page = 1, pageSize = 10) => {
  try {
    const response = await axiosInstance.get(
      `/Task/project/${projectId}/paginated?page=${page}&pageSize=${pageSize}`
    );
    return response.data;
  } catch (error) {
...
// GET: /api/Task/by-project/{projectId}
export const getAllTasksByProjectId = async (projectId) => {
  try {
    const response = await axiosInstance.get(`/Task/by-project/${projectId}`);
    return response.data; // This should be a List<TaskItemDto>
  } catch (error) {
    console.error(`Error fetching all tasks for project ID ${projectId}:`, error);
    if (error.response?.status === 401) {
      window.location.href = "/authentication/sign-in";
    }
    return [];
